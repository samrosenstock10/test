...
## Technical source lanes and future-leaders boundary

The existing Infrastructure schedule now uses 07:30 SemiAnalysis, 08:30 builder engineering (stable lane ID `engineering`) and 09:30 expert research (existing lane ID unchanged). Times are America/New_York. The source contract below owns exact research qualification and source completion. Private `samrosenstock10/muse-research-inbox/FUTURE_LEADERS.md` owns the joint future-leaders mandate. This document continues to own public schema integrity and protected publication.

- SemiAnalysis may contribute technical hypotheses to Expert Research/Evidence Register before a company beneficiary is known. `beneficiaryTickers: []` is valid. Its legacy company ledger retains the original narrower company-thesis qualification.
- Read complete current Research Settings before appending a missing `semianalysis` source registry record. Use a verified official public URL and preserve every existing source record exactly. Registry records, observations, evidence and reviews are immutable once published; do not relabel old evidence when changing the operating mandate.
- The engineering lane writes additive research, with an independent Research Runs key `engineering-YYYYMMDD` based on its New York logical date. Its completion cannot be inferred from the historical a16z run log or the 09:30 expert cycle. Preserve historical a16z ledgers, records, labels and signal types; do not insert engineering observations into the exact legacy a16z schema.
- Observation `sourceId` still identifies the original publisher/channel. Optional public metadata such as `sourceLane`, a public technical theme label, mechanism or measurable milestone may describe the observation. Coverage arrays reference actual registry IDs, not an unregistered scheduler lane. Deduplicate the originating item and independence group across lanes and peer write-ups.
- The private ChatGPT-owned `future-leaders/state.json` index, theme priorities, Muse `inbox/` case memos and ChatGPT `reviews/` dispositions reference canonical Sheet evidence rather than forming another public evidence database. Private peer exchanges, task state, file/repository links, personal context and financial-account details never enter candidate JSON, including optional metadata. Construct the public projection from canonical public research fields; do not copy private case objects and assume the URL validator will remove sensitive fields.
- Global public candidates at or below USD 50 billion total equity market capitalization at initial candidacy are the joint research focus, with no tiny-cap preference and no removal solely for subsequent growth. The threshold applies to private case selection, not deletion or filtering of existing public Company Universe/ranking history. Private businesses remain competitors, never invented public tickers.
- A private case disposition alone cannot change a public conviction score, valuation assumption or portfolio. Public ranked-company admission and changes retain the existing source, review, financial and protected-publication gates. A completed source scan can coexist with a separately due 09:30 case review; case work never claims source coverage or reruns a completed scan.

No schema-version change is required for these source-lane changes. Preserve complete immutable histories and both legacy feed files through the existing candidate workflow.
